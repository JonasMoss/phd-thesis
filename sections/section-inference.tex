...
theory.
\end{quote}
The case for Bayesianism is extremely strong \nptextcite{Berger1988-ji,Bernardo2009-wv}.
...
density estimation \nptextcite{Silverman1986-nt} do not involve likelihoods
...
AIC \nptextcite{Birge2006-nl}. But even in this case, no likelihoods are
...
The Robins--Wasserman does not matter to a Bayesian that firmly knows the likelihood and the prior. For him, the Bayes' theorem is the best you could possible do. And by Doob's consistency theorem \nptextcite{Miller2018-xq}, if you know your prior is true, every Bayesian procedure is consistent.
...
loss $l'(a,x)=b(e^{c(x-a)}-c(x-a)-1)$ \nptextcite{Varian1975-cd}, and
the Huber loss from robust statistics \nptextcite{Huber1964-fm}. For all
...
in fields such as psychology \nptextcite{Belia2005-di,Gigerenzer2018-oi}
and medicine \nptextcite{Goodman2008-ed,Gigerenzer2007-qi}. Frequentist quantities are almost never unique in the same
...
One reason why \emph{p}-values are not measures is evidence is the
lack of explicit alternative hypotheses. That \emph{p}-values are usually framed without explicit alternative hypotheses is sometimes
framed as a strength. \cite[p. 308]{Barnard1962-rz} wrote that ``the
simple tests of significance arise ,it seems to me, in situations
where we do not have a parameter space of hypotheses; we have only
a single hypothesis essentially, and the sample space then is the
only space of variables present in the problem.`` Fisher was firmly
against formal alternatives hypotheses and power calculations \nptextcite{Lehmann1993-oa}.
A compelling argument for why alternative hypotheses are important
is the Albino argument of \textcite{Berkson1942-hj}:
\begin{quotation}
Suppose I said, \textquotedblleft Albinos are very rare in human populations,
only one in fifty thousand. Therefore, if you have taken a random
sample of 100 from a population and found in it an albino, the population
is not human.\textquotedblright{} This is a similar argument but if
it were given, I believe the rational retort would be, \textquotedblleft If
the population is not human, what is it?\textquotedblright{} 
\end{quotation}
A major difference between the Neyman--Pearson theory of statistical
tests and p-values is the theoretical justification. Classical Neyman--Pearson-type
inferential statistics, with Lehmann's ``Testing statistical statistical
hypotheses'' \nptextcite{Lehmann2005-sp} as its bible, is concerned with
...
can be empty \nptextcite{Blaker2000-ud}. Confidence set for a real parameter
can contain the entire real line \nptextcite{Morey2016-ry}. Most confidence
sets are asymptotic, but with no guaranteed coverage since uniform
convergence almost never is proved, or even true \nptextcite{Gleser1996-kk}.
